...
import { calculatePF } from '../utils/pf';
...
const PfCalculator = () => {
  ...
  const validate = () => {
    const errs = {};
    if (!inputs.basicSalary || isNaN(inputs.basicSalary) || Number(inputs.basicSalary) <= 0) {
      errs.basicSalary = 'Enter a valid salary';
    }
    if (!inputs.employeePercent || isNaN(inputs.employeePercent) || Number(inputs.employeePercent) < 0) {
      errs.employeePercent = 'Enter a valid %';
    }
    if (!inputs.employerPercent || isNaN(inputs.employerPercent) || Number(inputs.employerPercent) < 0) {
    ...
    if (!inputs.years || isNaN(inputs.years) || Number(inputs.years) <= 0) {
